...
const outDir = process.argv[3] || path.join(__dirname, "..", "..", "..", "exchanges", "Pionex", "outputs", "qa", "vercel-french-pionex", target.startsWith("http") ? "public" : "local");
...
async function runOne(browser, name, viewport) {
  ...
  const report = await page.evaluate(() => {
    ...
    return {
      ...
      hasDisclosure: /affiliation|parrainage|commission|divulgation/i.test(visibleText),
      hasRisk: /risque|volatil|perte|liquidation/i.test(visibleText),
      hasRegionWarning: /France|Canada|juridiction|KYC|VPN|éligibilit/i.test(visibleText),
