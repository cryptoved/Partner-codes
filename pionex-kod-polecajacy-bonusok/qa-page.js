...
const outDir = process.argv[3] || path.join(__dirname, "..", "..", "..", "exchanges", "Pionex", "outputs", "qa", "vercel-polish-pionex", target.startsWith("http") ? "public" : "local");
...
async function runOne(browser, name, viewport) {
  ...
  const report = await page.evaluate(() => {
    ...
    const wordLike = (visibleText.match(/[A-Za-zÀ-ž0-9$%]+/g) || []).length;
    ...
    return {
      ...
      hasDisclosure: /afiliacyj|polecaj|prowizj/i.test(visibleText),
      hasRisk: /ryzyk|zmienno/i.test(visibleText),
      hasPoland: /Polska|Polsce|KNF|MiCA|UE/i.test(visibleText),
